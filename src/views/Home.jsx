...
export default function Home() {
  ...
  const [loadingTimelineById, setLoadingTimelineById] = useState({});
  ...
  // Función para controlar loader por máquina
  const setLoadingTimeline = (machineId, isLoading) => {
    setLoadingTimelineById((prev) => ({
      ...prev,
      [machineId]: isLoading,
    }));
  };
                  ...
                  {isExpanded && (
                    <div className="border overflow-scroll border-neutral-600 rounded flex flex-col items-center justify-center mt-1 p-2 w-full">
                      ...
                      {loadingTimeline ? (
                        ...
                        <TimelineRUNSTOP
                          key={`${machine.id}-${
                            timeRanges[machine.id] || "24h"
                          }`} // <-- agregar key dinámico
                          machineId={machine.id}
                          timeRange={timeRanges[machine.id] || "24h"}
                          onLoading={(isLoading) =>
                            setLoadingTimeline(machine.id, isLoading)
                          }
                        />
                      )}
                    </div>
